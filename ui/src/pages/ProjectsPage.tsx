...
export default function ProjectsPage() {
  ...
  useEffect(() => { loadProjects(); karateApi.listVersions().then(setKarateVersions).catch(() => {}) }, [])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await projectApi.create({ id: newProject.id, name: newProject.name, description: newProject.description, karateVersion: newProject.karateVersion })
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            {karateVersions.length > 0 && (
              <div>
                ...
                <select
                  value={newProject.karateVersion}
                  onChange={(e) => setNewProject({ ...newProject, karateVersion: e.target.value })}
                  className={inputCls}
                >
